...
class interfaceEngine:
    ...
    def run(self) -> dict:
        ...
        for col in self.column_profiles.items():
            if col_type == "numeric":
        ...
        for col in self.df.columns:
            ...
            def _analyse_numeric(self, col: str):
                 ...
                 self.insight.append({
                    ...
                    "finding": f"'{col}' is {skew_table}, skewed {direction} "
                    ...
                    outlier_count = lens(outliers)
                    outliers_pct = (outlier_counts / len(series)) * 100
...
# ── RULE: Normality Test (Shapiro-Wilk) ───────────────────────────────
        ...
        # we use D'Agostino-Pearson or Anderson-Darling.
                    ...
                    if len(series) < 5000:
                        stat, p_value = stat.shapiro(series)
             ...
             def _analyse_categorical(self, col: str):
                ...
                top_categoty = value_counts.index[0]
                ...
                if top_pct > 80:
                    self.insight.append({
                        ...
                        "finding": f"'{col}' is dominated by '{top_category}'"
                                   f"({cardinality_ratio*100:.1f}% unique). "
                                   f"This may be an ID or free-text column — consider excluding it from analysis.",
