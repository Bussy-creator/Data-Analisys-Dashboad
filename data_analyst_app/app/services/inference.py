...
class InferenceEngine:
    """
    Analyzes dataframes and generates statistical insights.
    Profiles columns, detects patterns, and finds relationships.
    """
    
    def __init__(self, df: pd.DataFrame):
        # stores the dataframe so every method can access it
        self.df = df
        ...
        # where we store each column's type 
    ...
    def _analyse_categorical(self, col: str):
        """
        Analyzes categorical columns for dominance, imbalance,
        ...
        if date_range > 0:
            ...
            self.insights.append({
                ...
                "finding": f"'{col}' spans {date_range} days from {min_date.date()} to {max_date.date()}. "
                           f"Missing {missing_dates} dates ({100-missing_pct:.1f}% coverage).",
                "values": {
